    ...
    <div className={`absolute inset-0 bg-field ${className}`}>
      ...
      <div className="absolute inset-4 flex flex-col justify-between border border-line p-4 text-foreground">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="bg-surface px-2 py-1 text-right text-xs font-black uppercase tracking-wide text-muted shadow-sm">{car.source}</span>
        </div>
        <div>
          <p className="text-xs font-black uppercase tracking-wide text-muted">{car.maker}</p>
          <p className="mt-1 text-2xl font-black leading-tight text-foreground">{car.model}</p>
          <p className="mt-2 text-xs font-bold uppercase tracking-wide text-muted">Auction photo pending</p>
        </div>
      </div>
    </div>
  );
