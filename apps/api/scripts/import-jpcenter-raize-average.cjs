...
const YEAR_TO = 2026;
const RAIZE_FREIGHT_JPY = 320_000;
const RAIZE_INSURANCE_JPY = 5_000;

const RAIZE_YELLOW_BOOK = {
  A210A: {
    freightJpy: 94_000,
    grades: { Z: 2_413_400, G: 2_235_200, X: 2_079_000 },
  },
  A202A: {
    freightJpy: 95_500,
    grades: { Z: 2_442_000, G: 2_263_800 },
  },
  A201A: {
    freightJpy: 95_500,
    grades: { Z: 2_152_700, G: 1_958_000, X: 1_800_700 },
  },
};
...
async function main() {
  ...
  for (let page = 1; page <= MAX_PAGES && imported.length < TARGET_CARS; page += 1) {
    ...
    for (const row of payload.body || []) {
      ...
      if (media.vehiclePhotos.length < 10 || !media.auctionSheet) {
        skipped.push({
          sourceUrl,
      ...
      const images = await saveSelectedMedia(
        [...media.vehiclePhotos.slice(0, 10), media.auctionSheet],
        imageFilePrefix('Raize', row.c || row.a),
      );
  ...
  if (imported.length < TARGET_CARS) {
    throw new Error(`Only ${imported.length} qualifying Raize records had complete media`);
  }
...
function buildCarDoc(row, sourceUrl, images, exchangeRate) {
  ...
  const grade = cleanDisplayText(row.l) || 'N/A';
  const averagePriceJpy = toNumber(row.o);
  const averageHistory = parseAverageHistory(row.p);
  const fuelType = modelCode === 'A202A' ? 'e-SMART Hybrid' : 'Petrol';
  const motorPowerKw = modelCode === 'A202A' ? 78 : undefined;
  const yellowBook = raizeYellowBookReference(modelCode, grade);
  const cost = calculateImportCost(
    {
      ...
      invoiceCifJpy: averagePriceJpy + RAIZE_FREIGHT_JPY,
      yellowBookValueJpy: yellowBook.valueJpy,
      yellowBookFreightJpy: yellowBook.freightJpy,
      depreciationRate: 0.85,
      freightJpy: RAIZE_FREIGHT_JPY,
      insuranceJpy: RAIZE_INSURANCE_JPY,
      vehicleType: 'Car',
      fuelType,
      engineCapacity,
      motorPowerKw,
      ...
      referenceSource: 'docs/Raize (2).xlsx',
      calculationBasis: 'JP Center Average Price with Raize Yellow Book reference',
    },
    DEFAULT_TAX_SETTINGS,
  );

  return {
    title: cleanDisplayText(`${year} Toyota Raize ${grade}`),
    maker: 'Toyota',
    model: 'Raize',
    modelCode,
    year,
    ...
    features: [
      ...
      averageHistory.length
        ...
        : '',
      `Grade ${grade}`,
      'Includes 10 vehicle photos and auction sheet',
    ].filter(Boolean),
    cost,
    status: 'available',
    published: true,
...
function raizeYellowBookReference(modelCode, grade) {
  const reference = RAIZE_YELLOW_BOOK[modelCode];
  if (!reference) return { valueJpy: undefined, freightJpy: undefined };
  const gradeKey = normalizeRaizeGrade(grade);
  return {
...
function normalizeRaizeGrade(grade) {
